...
description: >
  Analyzes unit economics by product or service from data the owner provides
  (CSV export, pasted numbers, or estimates), benchmarks against inflation and
  cost changes, and shows pricing-scenario data (e.g. "a 5% increase historically
...
2. **Gathers cost data** — COGS and direct expenses from the owner (CSV, export, or estimates)
3. **Gathers revenue data** — transaction history from the owner (CSV, export, or pasted numbers)
...
Ask the owner how they want to provide data:
- **CSV upload** — exported transaction and expense data. The expected CSV schema is in `reference/csv-schema.md`.
- **Pasted numbers** — revenue by product/service line and a cost breakdown
- **Estimates** — if no exports exist, rough figures per item are acceptable ("even rough figures work")
...
1. **"Which products or services do you want to analyze?"**
   ...
   - If they say "all," confirm the data has enough coverage to be meaningful before analyzing everything.
...
If they don't have cost data, surface this clearly:

> "To compute meaningful margins, I need a cost breakdown by product or service line — not a single average for the whole business. For each item you want analyzed, what does it cost you to deliver it? Materials, direct labor, any direct expenses per item. Even rough figures work."

Flag any missing cost data in the Data Quality Notes section of the final output.

### Step 4: Gather revenue data

Ask the owner for transaction history (CSV, export, or pasted):
- **Date range:** Match the cost data window (last 12 months, or full history if less is available)
...
- **Industry benchmarks:** Surface rough gross margin norms for the owner's category. See `reference/industry-benchmarks.md`.
